<template>
  <div class="w-full h-48 perspective-[1000px] xl:h-60" @click="fliped = !fliped">
    <div
      class="relative w-full h-full duration-500 preserve-3d rounded-xl outline-2" :class="{
        'rotate-y-180': fliped,
      ...
      <div
        ...
        <button class="absolute top-4 start-4 cursor-pointer" @click.stop="$emit('toggle-favorite', word)">
          <Icon
            name="i-uis:star" class="absolute hover:text-yellow-400 text-3xl" 
        ...
        <p
          class="font-bold absolute top-5 end-5" :class="{
      ...
      <div
        ...
        <button class="absolute top-4 start-4 cursor-pointer" @click.stop="$emit('request-updateWord', word)">
          <Icon name="i-mage:edit-pen" class="absolute hover:text-blue-300 text-xl lg:text-3xl z-50" />
        </button>
        <button class="absolute top-4 end-10 cursor-pointer" @click.stop="$emit('request-delete', word)">
